...
def crawl_articles(max_pages=2):
    """爬取文章列表 (JSON API)
    Args:
        max_pages: 爬取的最大页数
    """
    headers = get_headers()
    # 添加 JSON 请求所需的 headers
    ...
    for page in range(1, max_pages + 1):
        url = f"https://www.tgb.cn/newIndex/getZh?pageNo={page}"
        ...
        try:
            ...
            try:
                ...
                print(f"第 {page} 页响应不是有效的 JSON 格式")
                continue
        ...
        # 解析 JSON 结构
        # 结构参考: {"status":true, "dto": {"list": [...]}}
        if not data.get("status") or "dto" not in data or "list" not in data["dto"]:
            print(f"第 {page} 页返回数据结构不符合预期")
            continue

        articles = data["dto"]["list"]
        if not articles:
            print(f"第 {page} 页没有文章")
            break
        ...
        for item in articles:
            ...
            # 优先使用 newTopicID 构建新版链接 /a/{id}
            if new_topic_id:
                href = f"a/{new_topic_id}"
                full_url = urljoin(BASE_URL, href)
            # 降级使用 topicID 构建旧版链接 /bbs/{id}
            elif topic_id:
                href = f"bbs/{topic_id}"
                full_url = urljoin(BASE_URL, href)
            else:
        ...
        print(f"  第 {page} 页提取到 {page_count} 篇文章")
...
def save_to_json(data):
    ...
    # 前缀修改为 home_
    filename = "home_" + datetime.now().strftime("%Y-%m-%d") + ".json"
...
<body>
    <div class="container">
        <h1 class="doc-title">淘股吧文章汇总(Home)</h1>
    ...
    os.makedirs(output_dir, exist_ok=True)
    # 前缀修改为 home_
    filename = "home_" + datetime.now().strftime("%Y-%m-%d_%H%M%S") + ".html"
    filepath = os.path.join(output_dir, filename)
